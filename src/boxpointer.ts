...
export const FullscreenBoxpointer = registerClass(
	class FullscreenBoxpointer extends Semitransparent(St.Widget) {
		...
		constructor(arrow_side: St.Side, bin_properties?: Partial<St.Bin.ConstructorProps>) {
			...
			this.connect("notify::visible", () => {
				if (this.visible) global.compositor.disable_unredirect();
		...
		public override vfunc_captured_event(event: Clutter.Event) {
			...
			if (mute) return Clutter.EVENT_STOP;
		...
		public close(animate: PopupAnimation, on_complete: () => void) {
			...
			let translation_y = 0;
			const theme_node = this.get_theme_node();
			const rise = theme_node.get_length("-arrow-rise");
			const fade = animate & PopupAnimation.FADE;
			const animation_time = animate & PopupAnimation.FULL ? POPUP_ANIMATION_TIME : 0;
		...
		private _update_arrow_side(source_extents: Graphene.Rect, workarea: Mtk.Rectangle) {
			...
			switch (this._userArrowSide) {
				...
				case St.Side.BOTTOM: {
					...
					if (top_half_height > bottom_half_height) this._arrowSide = St.Side.BOTTOM;
					else this._arrowSide = St.Side.TOP;

					if (source_horizontal_center < workarea.x + workarea.width / 2)
						this.secondary_side = St.Side.LEFT;
					else this.secondary_side = St.Side.RIGHT;
				...
				case St.Side.RIGHT: {
					...
					if (left_half_width > right_half_width) this._arrowSide = St.Side.RIGHT;
					else this._arrowSide = St.Side.LEFT;

					if (source_vertical_center < workarea.y + workarea.height / 2)
						this.secondary_side = St.Side.TOP;
					else this.secondary_side = St.Side.BOTTOM;
